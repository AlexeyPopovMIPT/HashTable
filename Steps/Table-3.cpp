...
class HashTable
{
    ...
    struct TableElement : ListElement
    {
        ...
        int Hang(const char* val, const char* key)
        {
            ...
            return ! (this->tail->isValueDefined() && this->tail->isKeyDefined());
...
public:
    ...
    int Add (const char* new_elem, const char* key)
    {
        ...
        if (! target->isValueDefined())                                            /* если хеш раньше не встречался, изменяем TableElement в массиве */
        {
            target->makeFrom(new_elem);
            return ! target->isValueDefined();
        }
    ...
    {
        TableElement* target = table + getHashCode(key);
        if (! target->isValueDefined())
            return nullptr;

        if (! target->isNextDefined())
            return target->value;
    ...
    int ToJSON (const char* filename, const char* comma)
    {
        ...
        {
            fprintf(out,
            "        %d", now->isValueDefined() ? now->chainLength + 1 : 0);
