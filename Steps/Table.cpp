...
public:
    ...
    ~HashTable()
    {
        register TableElement* now = table;
        register TableElement* end = table + hashMaxValue;
        for (; now <= end; now++)
        {
            if (! (now->value))
                continue;
                
            register ChainElement* iter = now->next;
            register ChainElement* tail = iter;
            while (iter)
            {
                iter = iter->next;
                delete tail;
...
{
    HashTable table;
    table.setHashFunc(Polinomial_asm);
